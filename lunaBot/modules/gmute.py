...
from lunaBot import dispatcher, OWNER_ID, SUDO_USERS, SUPPORT_USERS, STRICT_GMUTE
from lunaBot.modules.helper_funcs.chat_status import user_admin, is_user_admin
from lunaBot.modules.helper_funcs.extraction import extract_user, extract_user_and_text
from lunaBot.modules.helper_funcs.filters import CustomFilters
from lunaBot.modules.helper_funcs.misc import send_to_list
from lunaBot.modules.sql.users_sql import get_all_chats
...
def check_and_mute(bot, update, user_id, should_message=True):
    if sql.is_user_gmuted(user_id):
        bot.delete_messages(update.effective_chat.id, user_id)
        if should_message:
